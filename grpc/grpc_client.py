...
def run():
    ...
    with grpc.insecure_channel('localhost:21337') as channel:
        ...
        response = stub.RegisterServer(registry_server_pb2.Server_information(name="Bankai", addr=str(__import__("time").time())))
        print("Grpc client [as a server] received: 1 " + str(response))
    with grpc.insecure_channel('localhost:21337') as channel:
        stub = registry_server_pb2_grpc.MaintainStub(channel)
        response = stub.GetServerList(registry_server_pb2.google_dot_protobuf_dot_empty__pb2.Empty())
